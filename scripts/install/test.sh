...
iam::test::user()
{
  ...
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/user/colin; echo
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/user/mark; echo
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/user/john; echo

  # 2. 创建 colin、mark、john 用户
  ${CCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/user \
    -d'{"password":"User@2021","metadata":{"name":"colin"},"nickname":"colin","email":"colin@foxmail.com","phone":"1812884xxxx"}'; echo

  # 3. 列出所有用户
  ${RCURL} "${token}" "http://${INSECURE_APISERVER}/v1/user?offset=0&limit=10"; echo

  # 4. 获取 colin 用户的详细信息
  ${RCURL} "${token}" http://${INSECURE_APISERVER}/v1/user/colin; echo

  # 5. 修改 colin 用户
  ${UCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/user/colin \
  ...
  # 7. 批量删除用户
  ${DCURL} "${token}" "http://${INSECURE_APISERVER}/v1/user?name=mark&name=john"; echo
...
iam::test::secret()
{
  ...
  # 1. 如果有 secret0 密钥先清空
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/secret/secret0; echo

  # 2. 创建 secret0 密钥
  ${CCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/secret \
    -d'{"metadata":{"name":"secret0"},"expires":0,"description":"admin secret"}'; echo

  # 3. 列出所有密钥
  ${RCURL} "${token}" http://${INSECURE_APISERVER}/v1/secret; echo

  # 4. 获取 secret0 密钥的详细信息
  ${RCURL} "${token}" http://${INSECURE_APISERVER}/v1/secret/secret0; echo

  # 5. 修改 secret0 密钥
  ${UCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/secret/secret0 \
...
iam::test::policy()
{
  ...
  # 1. 如果有 policy0 策略先清空
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/policy/policy0; echo

  # 2. 创建 policy0 策略
  ${CCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/policy \
  ...
  # 3. 列出所有策略
  ${RCURL} "${token}" http://${INSECURE_APISERVER}/v1/policy; echo

  # 4. 获取 policy0 策略的详细信息
  ${RCURL} "${token}" http://${INSECURE_APISERVER}/v1/policy/policy0; echo

  # 5. 修改 policy0 策略
  ${UCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/policy/policy0 \
...
iam::test::authz()
{
  ...
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/policy/authzpolicy; echo
  ...
  ${DCURL} "${token}" http://${INSECURE_APISERVER}/v1/secret/authzsecret; echo

  # 4. 创建 authzsecret 密钥
  secret=$(${CCURL} "${Header}" "${token}" http://${INSECURE_APISERVER}/v1/secret -d'{"metadata":{"name":"authzsecret"},"expires":0,"description":"admin secret"}')
