...
func Test_newSecrets(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "default",
			args: args{
				&service{store: mockFactory},
			},
			want: &secretService{store: mockFactory},
		},
	}
...
func (s *Suite) Test_secretService_Delete() {
	s.mockSecretStore.EXPECT().Delete(gomock.Any(), gomock.Eq("admin"), gomock.Eq("secret0"), gomock.Any()).Return(nil)
	type fields struct {
		store store.Factory
	}
	type args struct {
